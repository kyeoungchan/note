# 🧑🏻‍💻 복제
- [❗️ 레디스에서 복제 구조 세팅하기](#-레디스에서-복제-구조-세팅하기)
- [❗️ 레디스에서의 복제 구조](#-레디스에서의-복제-구조)
  - [✅ 복제 구조 구성하기](#-복제-구조-구성하기)
- [❗️ 복제 메커니즘](#-복제-메커니즘)
  - [✅ 비동기 방식으로 동작하는 복제 연결](#-비동기-방식으로-동작하는-복제-연결)
  - [✅ 복제 ID](#-복제-id)
  - [✅ 부분 재동기화](#-부분-재동기화)
  - [✅ Secondary 복제 ID](#-secondary-복제-id)
  - [✅ 읽기 전용 모드로 동작하는 복제본 노드](#-읽기-전용-모드로-동작하는-복제본-노드)
  - [✅ 유효하지 않은 복제본 데이터](#-유효하지-않은-복제본-데이터)
...
> 가장 상위의 노드인 마스터 노드만 데이터를 업데이트하는 커맨드를 수행할 수 있고, 하위 복제본은 모두 읽기 전용으로 동작하기 때문에 데이터를 읽어가는 커맨드만 수행할 수 있다.
...
![partial_resynchronization.png](../../res/partial_resynchronization.png)
...
![secondary_replication_id.png](../../res/secondary_replication_id.png)

> [!NOTE]
> 위 그림의 경우, 마스터 노드와의 복제가 끊어지자 복제본은 새로운 복제 ID를 갖게 되었다.  
> 만약 복제가 끊어진 뒤에도 노드 B가 기존의 복제 ID인 aaaaa를 유지하며 마스터로 동작하다가, 장애가 해결된 뒤 기존 마스터 노드였던 A와 다시 연결된다면 ➡ 두 노드는 동일한 복제 ID와 동일한 오프셋이 동일한 데이터셋을 갖는다는 사실읠 위반할 수 있기 때문이다.

<br>

> [!IMPORTANT]
> 노드 C가 노드 B에 복제 연결이 될 때, 두 노드의 master_replid2가 같기 때문에 노드 C는 노드 B에 부분 재동기화를 시도한다.  
> 노드 B의 백로그가 재동기화를 위한 데이터를 갖고 있는 경우라면, 부분 재동기화만으로 노드 C가 노드 B에 연결될 수 있다.  
> ➡️ 노드 B에서 RDB 파일을 백업 받아서 노드 C로 전송하는 전체 재동기화(full resynchronization)를 거치지 않아도 되기 때문에 불필요한 작업을 줄이고, 빠르게 복제 상태를 구축할 수 있다.
...
> ➡️ 복제본 노드의 데이터가 변경되더라도, 복제본이 재시작되거나 마스터와 전체 재동기화를 수행하면 복제보넹서 수행한 데이터는 사라진다.

<br>

> [!IMPORTANT]
> 복제본에 쓰는 내용은 오직 로컬에서만 유지되며, 해당 노드에 연결된 다른 복제본으로는 전파되지 않는다.  
> 만약 마스터 노드 A에 노드 B가 복제 연결, 노드 B에 노드 C가 복제 연결되어 있는 상태에서 노드 B를 조작하더라도,  
> 노드 C는 항상 노드 A와 동일한 데이터셋을 가진다.
...
> [!TIP]
> 복제 구조에서 유효하지 않은(stale) 데이터란 복제본의 데이터와 마스터의 데이터가 정확하게 일치하지 않는 경우의 데이터를 말한다.  
> `replica-serve-stale-data` 파라미터로 제어할 수 있는데,
> 1. `yes`(기본값)  
>    ➡ 복제본의 데이터가 유효하지 않다고 판단될 때에도 클라이언트로부터 들어오는 모든 읽기 요청에 데이터를 반환한다.
> 2. 'no'  
>    ➡ `INFO`, `CONFIG`, `PING` 등의 일부 기본 커맨드를 제외한 모든 커맨드에 대해 `SYNC with master in progress`라는 오류를 반환한다.
...
> 만약 백업 기능을 사용하지 않으려면 재부팅 후 레디스가 자동으로 재시작되지 않도록 설정할 것을 권장한다.  
> 
> 백업 기능을 사용하지 않을 때 만약 마스터 노드가 장애로 종료 후 재부팅됐을 때, 마스터 노드의 메모리는 초기화된다.  
> ➡️ 복제본 노드에는 이미 데이터가 존재하더라도, 마스터 노드로의 복제 연결을 시도하면서 빈 데이터 셋을 전달받게 된다.
> 
> 만약 백업을 사용했다면 레디스가 재부팅될 때 백업 파일을 자동으로 읽어오고, 복원된 내용이 복제본으로 전달된다.  
> 
> 만약 재시작 기능을 사용하지 않았다면 복제본 노드에는 데이터가 존재하기 때문에 마스터에서 복제본 노드로 변경하거나, 복제본 노드에서 데이터를 새로 백업받아 마스터 노드에 전달한 뒤 마스터 노드를 재부팅시키면 복제본 노드에 저장된 내용으로 데이터가 복원될 수 있다.
