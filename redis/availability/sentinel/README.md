...
- [✅ 센티널의 자동 페일오버 과정](#-센티널의-자동-페일오버-과정)
...
> [!NOTE]
> 현재 구성에 센티널 노드를 추가하는 것은 매우 간단하다.  
> 마스터를 모니터링하도록 설정한 센티널 인스턴스를 실행시키면 자동 검색 메커니즘에 의해 자동으로 기존에 실행 중이던 다른 센티널의 `known-list`에 추가된다.

<br>

> [!TIP]
> 만약 한 번에 여러 대의 센티널을 추가해야 하는 경우 10초 이상의 간격을 두면서 1개씩 목록에 들어가도록 천천히 추가하는 것이 오류의 발생 가능성을 줄일 수 있는 방법이다.  
> 모든 센티널의 추가가 끝났다면 `SENTINEL MASTER <mastername>` 커맨드의 `num-other-sentinels` 반환 값을 통해 센티널이 정상적으로 추가됐는지 확인한다.

<br>

> [!NOTE]
> 센티널을 제거하기 위해서는 앞에서 적었다시피 `SENTINEL RESET *`의 커맨드를 이용해 센티널이 모니터링하고 있는 정보를 리셋해야 한다.  
> 센티널 노드끼리는 오랜 시간동안 응답이 없어도 센티널의 `known-list`에서 지우지 않는다.

<br>

> [!TIP]
> 노드를 제거할 때는 인스턴스 사이에는 최소 30초 이상의 대기 시간을 가지며 차례대로 실행하는 것이 좋다.  
...
> PING에 대한 유효한 응답은 `+PONG`, `-LOADING`, `MASTERDOWN`이며, 다른 응답이나 응답을 아예 받지 못할 경우에는 모두 유효하지 않다고 판단한다.  

<br>

### ⭐️ sdown, odown 실패 상태로 전환
>[!NOTE]
> 하나의 센티널 노드에서 레디스 마스터 인스턴스에 대한 응답을 늦게 받으면 그 센티널은 마스터의 상태를 우선 sdown으로 플래깅한다.  
> `sdown`이란 `subjectly down`, 즉 주관적인 다운 상태를 의미한다.    
> 이후 센티널 노드는 다른 센티널 노드들에게 `SENTINEL is-master-down-by-addr <master-ip> <master-port> <current-epoch> <*>` 커맨드를 보내 다른 센티널에게 장애 사실을 전파한다.
...
> `odown`이란 `objectly down`, 즉 객관적인 다운 상태가 됐음을 의미한다.

<br>

> [!TIP]
> `odown` 플래그는 마스터 노드에 대해서만 갖는다.  
> 다른 복제본 노드에 장애가 발생하여 이를 인지한 경우, 해당 복제본 노드를 `sdown`으로 플래깅하지만, 해당 사실을 다른 센티널 노드로 전파하지도 않고, `odown`으로 변경하지도 않는다.  
> 다만 페일오버를 진행할 때 `sdown` 상태의 복제본은 마스터로 승격되도록 선택되지 않는다.
...
> [!NOTE]
> 처음으로 마스터 노드를 `odown`으로 인지한 센티널 노드가 페일오버 과정을 시작한다.  
> 센티널은 페일오버를 시작하기 전 우선 에포크(epoch) 값을 하나 증가시킨다.  
> 
> 에포크를 통해 각 마스터에서 발생한 페일오버의 버전을 관리한다.  
> 동일한 에포크 값을 이용해 페일오버의 과정이 진행되는 동안 모든 센티널 노드는 같은 작업을 시도하고 있다는 것을 보장할 수 있다.

<br>

### ⭐️ 센티널 리더 선출
> [!NOTE]
> 에포크를 증가시킨 센티널은 다른 센티널 노드에게 투표하라는 메시지를 보낸다.  
> 이때 증가시킨 에포크를 함께 전달하는데, 메시지를 수신한 센티널 노드가 현재 자신의 에포크보다 전달받은 에포크가 더 클 경우 잣니의 에포크를 증가싴니 뒤, 센티널 리더에게 투표하겠다는 응답을 보낸다.
> 
> 만약 전달받은 에포크 값이 잣니의 에포크 값과 동일하다면, 이미 리더로 선출한 센티널의 id를 응답한다.  
...
> 3. 2번 조건까지 동일하다면, runID가 샂너 순으로 작은 복제본
...
> [!NOTE]
> 스플릿 브레인이란, 네트워크 파티션 이슈로 인해 분산 환경의 데이터 저장소가 끊어지고, 끊긴 두 부분이 각각을 정상적인 서비스라고 인식하는 현상을 말한다.  
> 그러면 끊긴 두 부분에서 각각의 마스터 노드가 생겨나게 되고, 각 마스터 노드마다로 클라이언트 요청이 들어오면 두 데이터는 달라지게 된다.  
> ➡ 네트워크 단절이 복구된다면, 기존 마스터 노드는 새로 승격된 마스터 노드의 복제본으로 연될된다.  
> 그리고 복제본으로 연결되는 과정에서 기존에 처리했던 모든 데이터는 유실된다.
