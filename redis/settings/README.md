...
> ➡️ 서버 OS에서 **프로세스 하나가 열 수 있는 최대 FD 수 제한**이 있는데(`ulimit`), 이 값이 10,032보다 작으면 Redis는 그 한도에 맞게 `maxclients`를 자동으로 줄이는 것이다.
...
> [!TIP]
> 만약 레디스의 최대 클라이언트 수를 기본값인 10000으로 지정하고 싶다면, 서버의 파일 디스크립터 수를 최소 10032 이상으로 지정해야 한다.
...
```text
# *: 모든 OS 사용자에게 해당 규칙을 적용한다는 뜻
# hard: 일반 사용자 프로세스가 도달할 수 있는 최대 상한선
# soft: 프로세스가 실행될 때 실제로 적용되는 기본 제한 값. 일반 사용자가 `ulimit -Sn 100000` 등의 명령어로 늘릴 수 있다. 
# nofile: Number of Open Files. 동시 연결, 소켓 및 파일 핸들의 최대 개수
*               hard    nofile          100000
*               soft    nofile          100000
```
...
> 메모리 크기가 커질수록 페이지를 관리하는 테이블인 TLB(Translation Lookaside Buffer)가 감당해야할 페이지 테이블의 크기도 커져, 메모리를 사용할 때 오버헤드가 발생하는 이슈가 생긴다.  
> TLB의 크기가 매우 작기 때문에 메모리가 커질수록 페이지 수가 늘어나고 ➡ TLB Miss가 늘어나면서 ➡ 느려지게 되는 것이다.  
> 
> 페이지를 크게 만든 뒤 자동으로 관리하는 THP(Transparent Huge Page) 기능이 도입됐다.  
> ➡ 페이지 크기가 커지기 때문에 ➡ 페이지 수 감소, TLB Miss 감소
> 
> 하지만 **레디스와 같은 데이터베이스 애플리케이션에서는 오히려 이 기능을 사용할 때 퍼포먼스가 떨어지고 레이턴시가 올라가는 현상이 발생하기 때문에 레디스를 사용할 땐 이 기능을 사용하지 않는 것을 추천한다.**  
> Redis는 데이터 변경이 매우 잦고, Copy-on-Write(COW) 방식을 사용한다.  
> ❗️ 데이터를 복사할 때 실제로 바로 복사하지 않고, 수정이 발생하는 순간에만 그 부분을 복사하는 방식이다.  
> Redis는 디스크에 데이터를 저장할 때, fork()를 통해 자식 프로세스가 저장하는 동안 부모 프로세스는 계속 요청을 처리한다.  
> 데이터 수정이 발생한 경우에만 해당 페이지를 복사하는 방식을 통해 fork()를 지속적으로 사용해서 성능이 느려지는 것을 방지한다.  
> ➡️ COW는 페이지 단위로 복사를 하기 때문에 THP를 활성화하면 페이지 크기가 커져 복사 퍼포먼스가 느려지게 된다.
...
> - bind
>   - 기본값: 127.0.0.1 -::1
>     - `-`: 주소 바인딩에 실패하더라도 에러를 내지말고 조용히 넘어가라
>     - `::1`: IPv6의 로컬 호스트
>     - ➡️ IPv6 환경이 아니어서 바인딩에 실패하더라도, 로그에 경고만 남긴 채 IPv4(127.0.0.1)만 정상 연결하고 서버 실행
...
참고로, `redis-cli` 환경에서 한글이 깨져서 나오는데, `raw` 옵션을 통해 접속하면 깨지지 않는다.
```shell
$ redis-cli --raw
