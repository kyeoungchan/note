...
> [!TIP]
> 카프카의 스케일 아웃, 스케일 인 과정은 무중단으로 지원이 가능한 것이지, 자동으로 되는 것은 아니다.  
> 순수 카프카는 수동으로 브로커를 추가하거나 제거해야한다.  
> Cruise Control, Confluent Platform, AWS MSK는 자동 리밸런싱을 지원한다.
...
> 
> 물론 OS가 페이지 캐시에 쓰인 데이터를 비동기로 디스크에 쓰기 작업을 하기 때문에 OS ➡ 디스크로의 flush가 일어나기 전에 브로커 서버가 종료되거나 OS가 크래시가 일어나면 데이터가 유실될 수 있다. 
