...
>  토픽생성 시 파티션 개수 고려사항이다.

- [🚀 데이터 처리량](#-데이터-처리량)
- [🚀 메시지 키 사용 여부](#-메시지-키-사용-여부)
- [🚀 브로커, 컨슈머 영향도](#-브로커-컨슈머-영향도)
...
> ➡ 데이터를 처리함에 있어 지연 발생에 따른 서비스 영향도를 같이 고려하면서 파티션 개수를 정하는 것이 좋다.
...
> [!NOTE]
> 토픽의 압축 정책은 일반적으로 생각하는 zip이나 tar 압축(compression)과는 다른 개념이다.  
